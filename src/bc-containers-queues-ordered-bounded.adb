...
package body BC.Containers.Queues.Ordered.Bounded is
	...
	procedure Append (Q : in out Queue; Elem : Item) is
		Length : Natural := Queue_Nodes.Length (Q.Rep);
	begin
		--  FAST
		if Queue_Nodes.Available (Q.Rep) = 0 then
			if not (Elem < Queue_Nodes.Item_At (Q.Rep, Length)) then
				return;
			end if;
			for Index in reverse 1 .. Length - 1 loop
				if not (Elem < Queue_Nodes.Item_At (Q.Rep, Index)) then
					Queue_Nodes.Remove (Q.Rep, Length);
					Queue_Nodes.Insert (Q.Rep, Elem, Index);
					return;
				end if;
			end loop;
			Queue_Nodes.Remove (Q.Rep, Length);
			Queue_Nodes.Insert (Q.Rep, Elem, 1);
			return;
		end if;
		...
		--  SLOW
--		for Index in 1 .. Queue_Nodes.Length (Q.Rep) loop
--			if Elem < Queue_Nodes.Item_At (Q.Rep, Index) then
--				if Queue_Nodes.Available (Q.Rep) = 0 then
--					Queue_Nodes.Remove (Q.Rep, Queue_Nodes.Length (Q.Rep));
--				end if;
--				Queue_Nodes.Insert (Q.Rep, Elem, Index);
--				return;
--			end if;
...
--		end if;
	...
	procedure Insert (Q : in out Queue; Elem : Item) is
	begin
		Queue_Nodes.Remove (Q.Rep, Queue_Nodes.Length (Q.Rep));
		Append (Q, Elem);
	end Insert;
	...
	package Address_Conversions is new System.Address_To_Access_Conversions (Queue);
