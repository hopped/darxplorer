...
--  $Id: tree_test.adb 1391 2009-01-12 20:55:33Z simonjwright $
...
procedure Tree_Test is
   ...
   procedure Test_Binary_Iteration is
      T, T1, T2, T3, T4, T_glob : TB.Binary_Tree;
   ...
   begin
      ...
      Binary_Post_Order (T, Success);
		...
		Clear (T_Glob);
	  	declare
			T, T1, T2, Root : Binary_Tree;
	  	begin
		  Insert (T, '+', Left);
		  Root := Create (T);
		  
		  Append (T, 'a', Left, Left);
		  Append (T, 'b', Left, Right);

		  -- move down
		  Insert (T1, '+', Left);
		  Append (T1, 'b', Left, Left);
		  Append (T1, 'c', Left, Right);
		
		  Swap_Child (T, T1, Right);
		  Right_Child (T);
		  
		  Insert (T2, '+', Left);
		  Append (T2, 'c', Left, Left);
		  Append (T2, 'd', Left, Right);
			
		  Swap_Child (T, T2, Right);
