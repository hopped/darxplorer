--  $Id: tests-avl_trees.adb 1402 2009-02-22 11:48:45Z simonjwright $
--
--  Tests for AVL Trees.
...
package body Tests.AVL_Trees is
   ...
   begin
      ...
      Assert (Is_Null (T),
              "tree not null");
      Assert (not Is_Member (T, Item'('a', 0)),
              "('a', 0) is a member");
      declare
         Found : Boolean := True;
      begin
   ...
   begin
      ...
      Assert (Extent (T) = 1,
              "extent not 1");
      Assert (not Is_Null (T),
              "tree is null");
      Assert (Is_Member (T, Item'('a', 0)),
              "('a', 0) is not a member");
      Assert (Is_Member (T, Item'('a', 1)),
              "('a', 1) is not a member");
      Assert (Is_Member (T, Item'('A', 0)),
              "('A', 0) is not a member");
      Assert (not Is_Member (T, Item'('b', 0)),
              "('b', 0) is a member");
      Assert (not Is_Member (T, Item'('B', 0)),
              "('B', 0) is a member");
      declare
      ...
      begin
         ...
         Assert (Found,
                 "('a', 0) found on deletion");
         Assert (Extent (T2) = 0,
                 "extent not 0 after deleting ('a', 0)");
      end;
      declare
         T2 : AVL_Tree := T;
         Found : Boolean := False;
      ...
      Assert (not Not_Found,
              "('a', 1) was not found");
      Assert (Extent (T) = 1,
              "extent not 1 after inserting ('a', 1)");
      Not_Found := True;
      Insert (T, Item'('A', 2), Not_Found);
      Assert (not Not_Found,
              "('A', 2) was not found");
      Assert (Extent (T) = 1,
              "extent not 1 after inserting ('A', 2)");
   end One_Element;
